...
/**
 * Receive into an EXISTING destination row: add the goods and re-average cost.
 *
 * The weighted average is computed **inside the UPDATE**, from the row's own
 * committed values, rather than read into the application and written back.
 * That is what makes two receipts into the same row safe: the second one
 * averages against the first one's result instead of against a snapshot taken
 * before it happened. Read-modify-write here would silently lose one of them.
 *
 *   new cost = (quantity * cost + received * snapshot) / (quantity + received)
 *
 * `quantity` is the destination's PHYSICAL total, so stock the destination has
 * itself promised elsewhere still counts — it is owned, and it was bought at a
 * price that belongs in the average.
 *
 * The selling price is deliberately absent from this statement. It is
 * branch-private and does not travel with the goods.
 */
export async function receiveIntoExisting(
  tx: RawCapable,
  args: {
    companyId: Buffer;
    productId: Buffer;
  ...
  const changed = await tx.$executeRaw(Prisma.sql`
    UPDATE stock_items
       SET cost = (quantity * cost + ${args.received} * ${args.unitCost})
                  / (quantity + ${args.received}),
           quantity   = quantity + ${args.received},
           updated_at = NOW(6)
     WHERE company_id = ${args.companyId}
       AND product_id = ${args.productId}
       AND branch_id  = ${args.branchId}`);
