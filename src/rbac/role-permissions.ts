/**
 * THE canonical permission catalogue and role matrix.
 *
 * Reference data applied in ALL environments (R7; `price.edit` added in F1
 * Stage 2; the role -> permission matrix is D11).
 *
 * **This file lives under `src/` because runtime code depends on it.** It used
 * to sit in `prisma/seed-data/`, which `tsconfig.build.json` excludes from the
 * production build — so the seed could read the matrix but the SERVER could
 * not. That is the deeper reason self-service registration created roles with
 * no permissions at all: the matrix was somewhere registration could not reach.
 *
 * `prisma/seed-data/permissions.ts` now re-exports this module, so the seed and
 * every existing import keep working against exactly one definition. Do not
 * reintroduce a second copy of the matrix anywhere — `role-provisioning.spec.ts`
